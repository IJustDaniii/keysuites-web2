...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="detail-page">
    ...
    <section className="detail-title section-shell"><div><span className="property-type">{property.type} · {property.category}</span>{property.group && <Link href="/alojamientos/suites-jardines-de-la-alhambra" className="group-link">{copy.groupPrefix} {property.group} ↗</Link>}<h1>{property.name}</h1><p>{property.location}</p></div><Link href="#reserva" className="outline-button">{copy.availability} <span>↓</span></Link></section>
    ...
    <section className="location-section"><div className="section-shell location-inner"><div><span className="section-kicker">{copy.locationKicker}</span><h2>{property.location}</h2><p>{property.area}. {copy.privateAddress}</p></div><div className="map-placeholder"><span>KEY SUITES</span><i className="map-pin">●</i><strong>{property.area}</strong></div></div></section>
    ...
    <div className="mobile-booking"><a href="#reserva">{copy.availability} <span>↑</span></a></div>
  </main>;
}
