...
function PropertyGallery({ images, name }: { images: PropertyImage[]; name: string }) {
  return <section className={`gallery section-shell gallery-${Math.min(images.length, 2)}`} id="galeria">
    {images[0]
    ...
    <div className="gallery-side">
      {images[1] ? <div><Image src={images[1].src} alt={images[1].alt} fill sizes="30vw" /></div> : <PendingPhoto compact />}
      {images[2] ? <div><Image src={images[2].src} alt={images[2].alt} fill sizes="30vw" /></div> : <PendingPhoto compact />}
    </div>
...
function ReviewPanel({ item }: { item: CatalogItem }) {
  if (!REVIEW_MODE || item.warnings.length === 0) return null;
  return <aside className="review-panel">
    <div className="review-panel-head"><span>Modo revisión</span><strong>{item.warnings.length} puntos pendientes</strong></div>
    {item.warnings.map((note, index) => <ReviewNotice note={note} key={`${note.title}-${index}`} />)}
  </aside>;
}
...
function Reputation({ item }: { item: CatalogItem }) {
  const hasContent = item.platforms.length > 0 || item.positiveReviews.length > 0 || item.negativeReviews.length > 0;
  ...
  return <section className="reputation-section section-shell">
    ...
    <div className="reputation-content">
      {item.platforms.length > 0 && <div className="platform-list">{item.platforms.map((platform) => <article key={platform.name}>
        <strong>{platform.name}</strong><b>{platform.rating ?? 'Valoración no incorporada'}</b>
...
function RegistrationBlock({ item }: { item: CatalogItem }) {
  ...
  if (!hasRegistration) return REVIEW_MODE ? <div className="detail-info-card"><span>Registro turístico</span><p>Pendiente de información</p></div> : null;
...
function DetailInfo({ label, value }: { label: string; value: string | null }) {
  if (value === null && !REVIEW_MODE) return null;
  return <div className="detail-info-card"><span>{label}</span><p>{value ?? 'No encontrado · pendiente de información'}</p></div>;
}
...
function PropertyPage({ property }: { property: Property }) {
  ...
  const facts = [
    ...
    { label: 'Superficie', value: property.areaM2 === null ? null : `${property.areaM2} m²` },
  ].filter((fact) => fact.value !== null || REVIEW_MODE);
  ...
  return <main className="detail-page">
    ...
    <section className={`property-overview section-shell ${REVIEW_MODE && property.warnings.length ? '' : 'without-review'}`}>
      <div className="property-content">
        ...
        <div className="fact-row">{facts.map((fact) => <div key={fact.label}><strong>{fact.value ?? '—'}</strong><span>{fact.label}</span></div>)}</div>
        ...
        <div className="detail-info-grid">
          ...
          <DetailInfo label="Check-in" value={property.checkIn} />
          <DetailInfo label="Check-out" value={property.checkOut} />
        </div>
        ...
        <div className="address-registration"><DetailInfo label="Dirección publicada" value={property.address} /><RegistrationBlock item={property} /></div>
      </div>
...
function GroupPage({ group }: { group: PropertyGroup }) {
  ...
  return <main className="detail-page group-page">
    ...
    <section className={`group-intro section-shell ${REVIEW_MODE && group.warnings.length ? '' : 'without-review'}`}>
    ...
    <section className="group-information"><div className="section-shell">
      ...
      <div className="detail-info-grid">
        ...
        <DetailInfo label="Accesibilidad" value={group.accessibility} /><DetailInfo label="Check-in" value={group.checkIn} />
        <DetailInfo label="Check-out" value={group.checkOut} /><DetailInfo label="Dirección publicada" value={group.address} />
      </div>
