...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="detail-page">
    ...
    <section className="reviews-booking section-shell">
      ...
      <div className="booking-card" id="reserva"><span className="section-kicker">RESERVA TU ESTANCIA</span><h2>Consulta la disponibilidad</h2><p>Utiliza los enlaces confirmados de cada plataforma para consultar fechas y disponibilidad.</p>{property.platforms.map((platform) => platform.url ? <a className="platform-button" href={platform.url} target="_blank" rel="noreferrer" key={platform.name}>{platform.name}<small>Abrir plataforma ↗</small></a> : <span className="platform-button" key={platform.name}>{platform.name}<small>Enlace pendiente</small></span>)}{REVIEW_MODE && <div className="direct-pending">○ PENDIENTE: integrar motor de reservas directo</div>}</div>
    </section>
