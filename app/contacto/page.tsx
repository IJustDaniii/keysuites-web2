...
export default function ContactPage() {
  ...
  return <main className="contact-page section-shell">
    ...
    <section className="contact-card" aria-labelledby="contact-phone-title">
      ...
      <h2 className="visually-hidden" id="contact-phone-title">{copy.directTitle}</h2>
      <div className="contact-people">
        {copy.contacts.map((contact) => <article className="contact-person" key={contact.phoneDisplay}>
          <h3><a className="contact-phone" href={contact.phoneUrl} aria-label={`Llamar al ${contact.phoneDisplay}`}>{contact.phoneDisplay}</a></h3>
          <div className="contact-actions">
            <a className="contact-action contact-action-call" href={contact.phoneUrl} aria-label={`Llamar al ${contact.phoneDisplay}`}>
              <PhoneIcon />{copy.callButton}
            </a>
            <a className="contact-action contact-action-whatsapp" href={contact.whatsappUrl} target="_blank" rel="noopener noreferrer" aria-label={`Abrir WhatsApp para el ${contact.phoneDisplay}`}>
