...
export function Footer() {
  return <footer className="site-footer">
    <div className="footer-main section-shell">
      ...
      <div className="footer-col"><strong>{siteContent.footer.legalTitle}</strong><Link href="/politica-de-privacidad">Política de privacidad</Link><Link href="/aviso-legal">Aviso legal</Link>{siteContent.footer.legalItems.slice(2).map((item) => <span key={item}>{item}</span>)}{REVIEW_MODE && <em>{siteContent.footer.legalPending}</em>}</div>
    </div>
