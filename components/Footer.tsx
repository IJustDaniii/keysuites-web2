...
export function Footer() {
  return <footer className="site-footer">
    <div className="footer-main section-shell">
      ...
      <div className="footer-col"><strong>{siteContent.footer.brandTitle}</strong>{navigationLinks.slice(3).map((link) => <Link href={link.href} key={link.href}>{link.label}</Link>)}</div>
