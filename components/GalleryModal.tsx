...
export function GalleryModal({ images, propertyName }: { images: PropertyImage[]; propertyName: string }) {
  ...
  return <>
    <button className="gallery-button" type="button" onClick={() => setOpen(true)}>Ver todas las fotos <span>↗</span></button>
    <div className={`gallery-modal ${open ? 'open' : ''}`} aria-hidden={!open}>
      <button className="gallery-backdrop" type="button" aria-label="Cerrar galería" onClick={() => setOpen(false)} tabIndex={open ? 0 : -1} />
      <section role="dialog" aria-modal="true" aria-label={`Galería de ${propertyName}`}>
        <header><div><small>GALERÍA</small><h2>{propertyName}</h2></div><button ref={closeRef} type="button" onClick={() => setOpen(false)} aria-label="Cerrar galería">Cerrar <span>×</span></button></header>
